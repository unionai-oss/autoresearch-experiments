...
Tier C: frozen distilbert-base-uncased + linear probe. With N=50,000 samples and avg sequence length of 762 chars, the dataset is well above the 5k threshold for Tier C, making TF-IDF+LightGBM (Tiers A/B) too weak for this scale and text complexity. The auto domain lacks a well-established domain-specific BERT, so distilbert-base-uncased as a frozen encoder with a linear probe is the appropriate strong baseline before committing to full fine-tuning. Start here directly in experiment 0 — do not begin at a simpler tier.
