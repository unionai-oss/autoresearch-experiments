...
Tier C: frozen bert-base-uncased or distilbert-base-uncased linear probe (domain: auto → consider bert-base-uncased as no strong auto-specific BERT exists). With N=30,000 samples, TF-IDF tiers (A/B) are ruled out for NLP at this scale; Tier C frozen transformer linear probe is the correct starting point. The automotive domain lacks a widely adopted domain-pretrained BERT, so distilbert-base-uncased frozen + linear probe provides a strong, compute-efficient baseline before committing to full fine-tuning. Start here directly in experiment 0 — do not begin at a simpler tier.
