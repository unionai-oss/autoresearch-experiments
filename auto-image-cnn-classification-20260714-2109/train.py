...
# Define transforms (using detected native size)
...
# Full dataset (no transform yet — transforms applied via Subset wrappers later)
dataset = ImageFolder(DATA_PATH)

# Class mapping (ImageFolder already encodes classes to 0-based integers)
class_to_idx = dataset.class_to_idx
idx_to_class = {v: k for k, v in class_to_idx.items()}
print(f"[DATA] Class mapping (class -> index): {class_to_idx}")

# Extract all labels for stratified split
