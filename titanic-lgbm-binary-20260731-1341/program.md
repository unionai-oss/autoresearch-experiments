...
Tier A: LightGBM with optuna hyperparameter tuning (num_leaves, learning_rate, min_child_samples, feature_fraction). With only 891 samples, 11 features (6 numeric, 5 categorical), and a binary classification task, Tier A GBM is the clear choice — LightGBM handles mixed feature types natively and routinely dominates on small tabular datasets. Tiers B/C require 10k+ samples to justify the added complexity, and deep tabular models would overfit badly at N=891. Start here directly in experiment 0 — do not begin at a simpler tier.
