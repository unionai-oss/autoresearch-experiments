...
Tier C: EfficientNet-B2 full fine-tune with ImageNet pretrained weights, optional consideration of remote-sensing pretrained weights (e.g., BigEarthNet-pretrained ResNet34). With N=16,200 samples across 10 classes, the dataset falls squarely in the Tier C range (10k–50k). The domain is satellite/remote-sensing imagery (EuroSAT-style land-use classification), so a medium backbone like EfficientNet-B2 or ResNet34 fully fine-tuned is appropriate; lighter Tier B models would underfit given the dataset size, while heavier Tier D models would be overkill and slower to iterate on. Start here directly in experiment 0 — do not begin at a simpler tier.
