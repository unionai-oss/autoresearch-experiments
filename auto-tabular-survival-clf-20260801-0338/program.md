...
**Tabular strategy** (N=100 samples, F=11 features — 6 numeric, 5 categorical):
...
Tier A: LightGBM with optuna hyperparameter tuning (num_leaves, learning_rate, min_child_samples, feature_fraction) + CatBoost as secondary comparator for native categorical handling. With only 100 samples and 11 features (5 categorical, 6 numeric), this is firmly in Tier A territory; GBMs handle small tabular datasets well without overfitting risk that deeper models would face, and CatBoost's native categorical encoding is especially valuable given the 5 categorical columns without needing heavy preprocessing. Start here directly in experiment 0 — do not begin at a simpler tier.
...
Up to 3 experiments (0-indexed). The runner stops early based on your DESCRIPTION or if you return STOP.
