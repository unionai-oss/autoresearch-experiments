...
- Classes: 3
- Class distribution: {"0": 24509, "1": 24348, "2": 1143}
- Missing rate: 0.0%
- Target: `label`
- Domain: auto
...
Tier B: LightGBM with k-mer TF-IDF (ngram 3-6). The domain is 'auto' (automotive text/sequences, not biological), avg length of 762 chars is manageable with TF-IDF, and N=50k with only 3 classes is well-suited for gradient boosting on sparse n-gram features. Long-range transformer models (Tier D) are unnecessary for an automotive classification task where local n-gram patterns are typically sufficient, and the class imbalance (class 2 at ~2.3%) can be handled via LightGBM's class_weight. Start here directly in experiment 0 — do not begin at a simpler tier.
