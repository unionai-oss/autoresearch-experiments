...
**Tabular strategy** (N=891 samples, F=11 features — 6 numeric, 5 categorical):
...
Tier A: LightGBM with optuna hyperparameter tuning (num_leaves, learning_rate, min_child_samples, feature_fraction). With only 891 samples and 11 mixed-type features, this is a classic small tabular binary classification problem firmly in Tier A; LightGBM handles the 5 categorical features natively and is well-proven on datasets of this size (this appears to be Titanic-scale), making deeper models unnecessary and prone to overfitting. Start here directly in experiment 0 — do not begin at a simpler tier.
